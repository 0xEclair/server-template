...
func (s *AssetsListService) ListWithOssAndBRC420V4() serializer.Response {
	var assets []model.Asset
	...
	dlcRes := dlcService.ListDLCWithOssAndBRC420V4()
	dlcList, ok := dlcRes.Data.(*serializer.AssetsListResponse)
	if ok {
		for _, item := range dlcList.Items {
			if item.InscriptionId == "e80eadea13e2175949168c279f7a47a467b6c5e00a84d45dd8ae40aefda89fe5i0" {
	...
	var aas []model.AssDLC
	var exist = make(map[string]bool)
	for _, a := range as {
		if exist[a.InscriptionId] {
			continue
		}
		aas = append(aas, a)
		exist[a.InscriptionId] = true
	}
...
func (s *AssetsListService) ListWithOssAndBRC420V5() serializer.Response {
	if s.Collection == "others" {
		var assets []model.Asset
		config.Postgres.Model(&model.Asset{}).Where("address = ? and cons = true", s.Address).Order("id").Find(&assets)
...
func (s *AssetsListService) ListDLCWithOssAndBRC420V4() serializer.Response {
	var assets []model.Asset
